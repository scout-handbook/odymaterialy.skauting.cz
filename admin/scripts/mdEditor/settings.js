function MDlessonSettings(id, actionQueue, noHistory)
{
	sidePanelOpen();
	var html = "<div class=\"button yellowButton\" id=\"sidePanelCancel\"><i class=\"icon-right-open\"></i>Zavřít</div>";
	html += MDrenderField();
	html += MDrenderCompetences();
	html += MDprerenderGroups();
	document.getElementById("sidePanel").innerHTML = html;
	MDlessonSettingsCacheEvent.addCallback(renderGroups);
...
function MDrenderField()
{
	...
	if(MDlessonSettingsCache.field === "")
	{
	...
	else
	{
		...
		{
			if(FIELDS[i].id && FIELDS[i].id === MDlessonSettingsCache.field)
			{
				html += FIELDS[i].name;
				break;
			}
...
function MDrenderCompetences()
{
	...
	{
		if(MDlessonSettingsCache.competences.indexOf(COMPETENCES[i].id) >= 0)
		{
...
function renderGroups()
{
	...
	{
		if(MDlessonSettingsCache.groups.indexOf(GROUPS[i].id) >= 0)
		{
